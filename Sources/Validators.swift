...
func validateIPv4(_ value:Any) -> ValidationResult {
  if let ipv4 = value as? String {
    var sin = sockaddr_in()
    return withUnsafeMutablePointer(to: &sin.sin_addr, { (address) -> ValidationResult in
      if ipv4.trimmingCharacters(in: .whitespaces).withCString({ cstring in inet_pton(AF_INET, cstring, address) }) == 1 {
        return .valid
      }
...
func validateIPv6(_ value:Any) -> ValidationResult {
  if let ipv6 = value as? String {
    ...
    return withUnsafeMutablePointer(to: &sin.sin6_addr, { (address) -> ValidationResult in
      if ipv6.trimmingCharacters(in: .whitespaces).withCString({ cstring in inet_pton(AF_INET6, cstring, address) }) == 1 {
        return .valid
      }
